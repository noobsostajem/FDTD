...
factor = 4.0 # for slab lambda/2
epsilon_2=(1.0/int(wavelength*np.sqrt(epsilon_free)/np.sqrt(epsilon_2)/factor)*wavelength*np.sqrt(epsilon_free)/factor)**2
epsilon_3=(epsilon_2/np.sqrt(epsilon_free))**2
eps= np.ones(size)
eps[:] = epsilon_free
plate_thickness=wavelength*mt.sqrt(epsilon_free)/mt.sqrt(epsilon_2)/factor
eps[int(size/2.0):int(size/2.0)+int(wavelength*np.sqrt(epsilon_free)/mt.sqrt(epsilon_2)/factor)] = epsilon_2
eps[int(size/2.0)+int(wavelength*np.sqrt(epsilon_free)/mt.sqrt(epsilon_2)/factor):] = epsilon_3
...
################################PRINT#####
plt.subplot(3, 1, 1)
plt.axvline(size/2, color='r',linewidth=2)
plt.axvline(size/2+int(plate_thickness), color='r',linewidth=2)
eZ=plt.plot(range(0,size,1),ez,'-',color='b', label="Ez")
hY=plt.plot(range(0,size,1),ez_free,'-',color='g', label="Ez_free")
